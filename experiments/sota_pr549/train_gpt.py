...
def _sdpa_flash_compat(q, k, v, causal=False):
    """Drop-in replacement for flash_attn_3_func using PyTorch SDPA.
    flash_attn uses (B, T, H, D) layout; SDPA uses (B, H, T, D)."""
    q_t = q.transpose(1, 2)
    k_t = k.transpose(1, 2)
    v_t = v.transpose(1, 2)
...
flash_attn_3_func = _sdpa_flash_compat
